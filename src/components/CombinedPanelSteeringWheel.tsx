...
export default React.memo(function CombinedPanelSteeringWheel() {
  ...
  // Unified panel position storage
  const [unifiedPanelPosition, setUnifiedPanelPosition] = useStorage<{x: number, y: number}>(
    "unifiedPanelPosition", 
    { x: 20, y: 20 }
  );
    ...
    <>
      ...
      <button 
        onClick={() => {
          ...
          // Just a state update to trigger re-render
  ...
  // Define all sections for the unified panel
  const sections = [
    {
      id: 'wheelModel',
      title: 'Wheel Model',
  ...
  return (
    <div className="w-screen h-screen bg-black fixed top-0 left-0">
      ...
      {/* Unified Control Panel with all sections */}
      <UnifiedControlPanel
        title="Steering Wheel Controls"
        componentName="SteeringWheel"
        panelId="unified"
        initialPosition={unifiedPanelPosition}
        onPositionChange={setUnifiedPanelPosition}
        initialCollapsed={false}
        theme="dark"
        initialWidth={350}
        initialHeight="auto"
