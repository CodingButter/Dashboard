...
import UnifiedSteeringWheel from "./components/UnifiedSteeringWheel"
...
function UnifiedApp() {
  ...
  const [unifiedInterface, setUnifiedInterface] = useState<boolean>(false)
  ...
  return (
    <div className="w-full h-full">
      ...
      <div className="absolute top-0 left-0 z-10 flex gap-2 p-2 bg-black/40 rounded-br shadow-lg">
        ...
        {/* Interface toggle - only available for wheel */}
        {activeComponent === "wheel" && layout === "single" && (
          <div className="flex items-center ml-4 border-l pl-4 border-gray-600">
            <button
              onClick={() => setUnifiedInterface(false)}
              className={`px-3 py-1 rounded ${!unifiedInterface ? "bg-purple-600" : "bg-gray-700"}`}
              title="Use multiple panels"
            >
              Multi-Panel
            </button>
            <button
              onClick={() => setUnifiedInterface(true)}
              className={`px-3 py-1 ml-2 rounded ${unifiedInterface ? "bg-purple-600" : "bg-gray-700"}`}
              title="Use unified panel with sections"
            >
              Unified Panel
            </button>
          </div>
      ...
      {layout === "single" ? (
        ...
        <div className="w-full h-full">
          {activeComponent === "wheel" && (
            unifiedInterface ? <UnifiedSteeringWheel /> : <SteeringWheel />
          )}
