import CombinedPanelSteeringWheel from "./components/CombinedPanelSteeringWheel"
...
function CombinedApp() {
  ...
  const [useCombinedPanel, setUseCombinedPanel] = useState<boolean>(true)
  ...
  return (
    <div className="w-full h-full">
      ...
      <div className="absolute top-0 left-0 z-10 flex gap-2 p-2 bg-black/40 rounded-br shadow-lg">
        ...
        {/* Panel style toggle - only available for wheel */}
        {activeComponent === "wheel" && layout === "single" && (
          <div className="flex items-center ml-4 border-l pl-4 border-gray-600">
            <button
              onClick={() => setUseCombinedPanel(false)}
              className={`px-3 py-1 rounded ${!useCombinedPanel ? "bg-purple-600" : "bg-gray-700"}`}
              title="Use multiple separate panels"
            >
              Separate Panels
            </button>
            <button
              onClick={() => setUseCombinedPanel(true)}
              className={`px-3 py-1 ml-2 rounded ${useCombinedPanel ? "bg-purple-600" : "bg-gray-700"}`}
              title="Use a unified panel with sections"
            >
              Combined Panel
            </button>
          </div>
      ...
      {layout === "single" ? (
        ...
        <div className="w-full h-full">
          {activeComponent === "wheel" && (
            useCombinedPanel ? <CombinedPanelSteeringWheel /> : <SteeringWheel />
          )}
