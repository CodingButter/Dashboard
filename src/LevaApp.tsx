import LevaSteeringWheel from "./components/LevaSteeringWheel"
...
function LevaApp() {
  ...
  const [useCombinedLevaPanel, setUseCombinedLevaPanel] = useState<boolean>(true)
      ...
      <div className="absolute top-0 left-0 z-10 flex gap-2 p-2 bg-black/40 rounded-br shadow-lg">
        ...
        {activeComponent === "wheel" && layout === "single" && (
          <div className="flex items-center ml-4 border-l pl-4 border-gray-600">
            <button
              onClick={() => setUseCombinedLevaPanel(false)}
              className={`px-3 py-1 rounded ${!useCombinedLevaPanel ? "bg-purple-600" : "bg-gray-700"}`}
              title="Use separate custom panels"
            >
              Custom Panels
            </button>
            <button
              onClick={() => setUseCombinedLevaPanel(true)}
              className={`px-3 py-1 ml-2 rounded ${useCombinedLevaPanel ? "bg-purple-600" : "bg-gray-700"}`}
              title="Use combined Leva panel"
            >
              Leva Panel
            </button>
          </div>
      ...
      {layout === "single" ? (
        ...
        <div className="w-full h-full">
          {activeComponent === "wheel" && (
            useCombinedLevaPanel ? <LevaSteeringWheel /> : <SteeringWheel />
          )}
